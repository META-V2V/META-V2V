...
def main_soundness(bk_type: str, cxpb: float, mutpb: float):
    ...
    file_path = os.path.join(RECORDS_DIR, timestamp,
                             BK_FILE_MAP.get(bk_type) + '_' + 'soundness_single' + '.csv')
    ...
    # stage 1
    toolbox.register('evaluate', partial(eval_stage_1, timestamp=timestamp, param_list=param_list, bk_type=bk_type, mode='soundness_single'))
    ...
    for ind, fit in zip(invalid_ind, fitnesses):
        # the min_dist of source scenario
    ...
    toolbox.register('evaluate', partial(eval_stage_2, timestamp=timestamp, param_list=param_list, bk_type=bk_type, mode='soundness_single'))
    ...
    while True:
        ...
        fitnesses = toolbox.map(toolbox.evaluate, invalid_ind)
        for ind, fit in zip(invalid_ind, fitnesses):
            # single objective optimization, minimize min(dist[0]) - max(diff[1:])

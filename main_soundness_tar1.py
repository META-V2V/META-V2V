...
def main_soundness(bk_type: str, cxpb: float, mutpb: float):
    ...
    file_path = os.path.join(RECORDS_DIR, timestamp,
                             BK_FILE_MAP.get(bk_type) + '_' + 'soundness_tar1' + '.csv')
    ...
    # stage 1
    toolbox.register('evaluate', partial(eval_stage_1, timestamp=timestamp, param_list=param_list, bk_type=bk_type, mode='soundness_tar1'))
    ...
    toolbox.register('evaluate', partial(eval_stage_2, timestamp=timestamp, param_list=param_list, bk_type=bk_type, mode='soundness_tar1'))

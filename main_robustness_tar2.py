...
def main_robustness(bk_type: str, cxpb: float, mutpb: float):
    ...
    file_path = os.path.join(RECORDS_DIR, timestamp,
                             BK_FILE_MAP.get(bk_type) + '_' + 'robustness_tar2' + '.csv')
    ...
    # only stage 2
    toolbox.register('evaluate', partial(eval_stage_2, timestamp=timestamp, param_list=param_list, bk_type=bk_type, mode='robustness_tar2'))

...
class MapParser:
    """
    Class to load and parse HD Map, using pickle dict to cache the instance for better performance
    ...
    __instance: Dict[str, 'MapParser'] = dict()
    ...
    def get_instance(map_name: str) -> 'MapParser':
        ...
        param: str map_name: The name of the map in Apollo data
        returns: MapParse instance
        rtype: MapParser
        """
        map_dir = Path(MT_ROOT, 'hdmap', 'maps')
    ...
    def parse_signal_relations(self) -> None:
        """
    ...
    def parse_lane_relations(self) -> None:
        """
    ...
    def get_coordinate_and_heading(self, lane_id: str, s: float) -> Tuple[PointENU, float]:
        ...
        # lst.coords[:-1] get the second to last element, (A, B, C, D) -> (A, B, C)
        # lst.coords[1:] get the second element, (A, B, C, D) -> (B, C, D)
        # construct a list of LineString tuples [(A, B), (B, C), (C, D)]
        segments.sort(key=lambda x: ip.distance(x))
        # sort the segments, choose the segment closest to the ip interpolation point
        line = segments[0]
        x1, x2 = line.xy[0]
        y1, y2 = line.xy[1]
        # from the segment closest to the ip interpolation point, extract the xy coordinates of the end and start points

        return (PointENU(x=ip.x, y=ip.y), math.atan2(y2-y1, x2-x1))
        # this part uses the differential method to solve the direction angle
    ...
    def get_lanes_not_in_junction(self) -> Set[str]:
        ...
        for junc in self.__lanes_at_junction:
            # iterate over the lane and junction relation parsed before
    ...
    def get_path_from(self, lane_id: str) -> List[List[str]]:
        ...
        """
        for _ in range(5):
            # try 5 times to find the path

            # get all possible paths starting from the specified lane
            target_lanes = self.get_lanes_not_in_junction()
            # get all lanes that are not in the junction
            reachable = self.__find_paths(lane_id)
            # recursively get all possible paths starting from the specified lane
            result = [p for p in reachable if p[-1] in target_lanes]
            if len(result) > 0:
                # if there is a feasible path, return the result
                return result
        return []

    def __find_paths(self, lane_id: str, depth:int=5) -> List[List[str]]:
        """
        New implementation of finding paths from a starting lane

        note: we've fixed the bug in the original DoppelTest code with improper implementation of depth-first search
        ...
        :rtype: List[List[str]]
        """
        depth -= 1     # preprocess the depth, because the depth starts from 0 in dfs
